...
import { dialogModes, editCategories, editModes, EditTargets } from "./constants";
export class UI {
    static editMode;
    static editCategory;
    static editTarget;
    ...
    static trigger(code1, code2, code3) {
        this.editMode = editModes[-(code1 + 6)];
        this.editCategory = editCategories[(code2 - 48)];
    ...
    static async merge(srcID, overwrite) {
        if (!(await Experiment.isBuilt(srcID))) {
            return `experiment ${srcID} does not exist`;
        }
        let src_bp = await require(paths.blueprint(srcID));
        for (let component in src_bp) {
            ...
            if (component === 'syntax') {
                ...
                for (let key in componentObject) {
                    setValue.call(Experiment.blueprint, component, key, componentObject[key], overwrite);
                }
            }
        }
    }
}
function viewProp(tgt) {
...
function* indexInputDialog(src, tgt) {
    ...
    if (count === 0)
        return [1, `there are no ${src}s to associate the ${tgt} with`];
    while (true) {
