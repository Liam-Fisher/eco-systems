...
export default class Interaction {
    ...
    static async addMethod(interactionIDs, ...systemIDs) {
        let ID = interactionIDs.pop();
        if (!ID || this.interactions.has(ID))
            return;
        let interactionModule = await require(paths.subroutine(ID, 'interactions'));
    ...
    static addUndefined(systemProps, ...systemIDs) {
        for (let systemID of systemIDs) {
            cloneRecurse(Production.systems.get(systemID), systemProps);
        }
    }
    static buildGenerator(interactionID, ...systemIDs) {
        ...
        if (difference.length && this.generators.has(interactionID))
            return;
    ...
    static interact(interactionIDs) {
        if (!interactionIDs.length || this.max === this.count++)
            return;
        let ID = interactionIDs.pop();
        ...
        if (!generated.value || !generated.value.length)
            return;
