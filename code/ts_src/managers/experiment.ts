...
export class Experiment {
    static id: string
    static blueprint:  experiment_blueprint;
    static async run(ID: string, generations?: number) {
        await this.load(ID);
        await this.build();
        Control.iter(generations);
    }
    static async store(replace: boolean|0|1) {
    ...
    static async isBuilt(ID: string) {
        this.id = ID;
    ...
    static async load(ID: string) {
        ...
        this.blueprint = await require(paths.blueprint(ID)) as experiment_blueprint;
    ...
    static async rename() {
        ...
        for (let blueprint of existingBlueprints) {
            ...
            if (match[1] === this.id) maxIndex = Math.max(maxIndex, +match[2]);
        }
