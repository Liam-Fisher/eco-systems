...
export default class InterpretationDebugger extends Debugger{
    targets: interpretationStage[]|'all'
    constructor(path: string, targets?: interpretationStage[]|boolean){
        super(path);
    ...
    link(dequeuedMsg: SystemClass[]) {
        return `linking systems: \n ${dequeuedMsg.map(s => `data: \n${this.sData(s)}props: \n${this.sProps(s)}`).join('\n')}`
    }
    addMethod(isSequence: boolean, module: string, target: string, letter: string, ...methodIDs: string[]) {
        let data = `adding ${isSequence?'sequence':'group'} methods from module ${module} for `;
        if(isSequence) data +=`system ${target} at letter ${letter}: `;
        else data += `word ${target}: `;
        data += `${methodIDs.join(', ')}\n`;
        return data;
    }
    logText(stage: interpretationStage, dequeuedMsg: SystemClass[], info: string[]) {
