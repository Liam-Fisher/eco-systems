...
//import Debugger from "../main/debugger";
// implementation specific formatting

// only a single channel can be used per system when streaming. multiple channels 
export async function* format(source: AsyncIterable<(string|number)[]>) { 
    ...
    for await (let methodResult of source) {
        if(!methodResult?.length||((++this.sampleIndex)>=this.attrs.samples)) continue;
