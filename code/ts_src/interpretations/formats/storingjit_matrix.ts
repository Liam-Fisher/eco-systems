...
export async function* format(source: AsyncIterable<number[][]>) {
    ...
    this.data.writeUInt32BE(this.filesize, 4); // filesize
    this.data.write("JIT!", 8); 
    this.data.write("FVER", 12); 
    this.data.writeUInt32BE(12, 16); 
    this.data.writeUInt32BE(1016323200, 20); // version code?? timestamp??
    this.data.write("MTRX", 24); 
    this.data.writeUInt32BE(this.filesize-24,28); // chunksize
    this.data.writeUInt32BE(this.offset-24,32); // bytes before matrix data
    this.data.write(this.attrs.type,36); // type code
    this.data.writeUInt32BE(this.attrs.planecount,40); // planecount
    this.data.writeUInt32BE(this.attrs.dim.length,44); // dimcount 
    this.attrs.dim.forEach((d, i) => this.data.writeUInt32BE(d, 48+i*4));
    
    for await (let methodResult of source) {
        if (!(Array.isArray(methodResult[0]) && Array.isArray(methodResult[1]))) continue;
        ...
        for (let planeIndex = 0; planeIndex < this.attrs.planecount; planeIndex++) {
            ...
            if(this.type ==="CHAR") this.data.writeUInt8(value, offset);
            else if(this.type ==="FL32") this.data.writeFloatBE(value, offset);
            else if(this.type ==="LONG") this.data.writeUInt32BE(value, offset);
            else this.data.writeDoubleBE(value, offset);
        }
    };
