...
///(?<ErrorType>\w+)Error: /g
// new RegExp(`(?<ErrorType>\w+)Error: `, 'g');
     ...
     for await (const match of eStack.matchAll(errorLocation)) {
          for (const prop in match.groups) {
               if (match.groups[prop]) {
                    if (parsedError[prop]) parsedError[prop].push(match.groups[prop]);
                    else parsedError[prop] = [match.groups[prop]];
               }
          }
     };
...
export class Debugger {
     ...
     lineSymbols: Record<string, string[]>;
     ...
     pWord(letters: string, parameters: number[][]) {
          let data = '';
          let line = `(0) `;
     ...
     sProps(system: SystemClass) {
          let propCount = 0;
          let propStrings = ``;
          for (let key in system) {
               if (['id', 'axiom', 'schema', 'letters', 'parameters', 'parameterSize', 'maxLetters', 'age', 'terminalAge'].includes(key)) continue;
               let val = system[key];
               if (Array.isArray(val)) val = val.join(' ');
               propStrings += `(${propCount++})\t${key}: ${val}\n`;
          }
          if (!propCount) return `${system.id} has no props\n`
     ...
     line(key: string, position?: 'begin' | 'end' | 'separate') {
          let sym = '_';
...
/*
function scheduleLog(component: string, componentID: string, ...systemIDs: string[]){
    let data = `at generation ${Control.age} of ${Control.terminalAge}\n`;
    data += `${'+'.repeat(256)}\n`;
    data += `applying ${component} "${componentID}" to systems: \n${systemIDs.join(', ')}\n`;
    this.queue.push(data);
    if(this.ready)  {
    this.dequeue();
    }
...
export const testForUniqueIDs = function(b: Blueprint){
     let sysIDs: string[] = b.systems.map(sys => sys.id);
     let netIDs: string[] = b.networks.map(net => net.id);
     let allIDs: Set<string> = new Set([...sysIDs.concat(netIDs)]);
     if(allIDs.size !== (sysIDs.length+netIDs.length)) {
         throw new Error(`duplicate ids in experiment`);
     }
 }
 */